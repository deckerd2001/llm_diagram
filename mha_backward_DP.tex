...
\node[align=center, left=1.0cm of dWO_calc, color=red!70]
...
\node[align=center, color=red!70] (dBO) [below=0.7cm of Sum_dBO] {$\mathbf{d}\widetilde{\mathbf{b}}_{O}$\\$[D]$};
...
\node[align=center, color=red!70] (dWV_out) [left=1.0cm of dWV_mul] {$\mathbf{d}\widetilde{\mathbf{W}}_{V}$\\$[D,D]$};
\draw[dpgradweight] (dWV_mul.west) -- (dWV_out);
...
\draw[gradflow] (T_XnormQ.west) -- (dWQ_mul.east)
  node[dimlabel, midway, above]{$[B,D,S]$};
\node[align=center, color=red!70] (dWQ_out) [left=1.0cm of dWQ_mul] {$\mathbf{d}\widetilde{\mathbf{W}}_{Q}$\\$[D,D]$};
\draw[dpgradweight] (dWQ_mul.west) -- (dWQ_out);
...
\draw[gradflow] (T_XnormK.west) -- (dWK_mul.east)
  node[dimlabel, midway, above]{$[B,D,S]$};
\node[align=center, color=red!70] (dWK_out) [left=1.0cm of dWK_mul] {$\mathbf{d}\widetilde{\mathbf{W}}_{K}$\\$[D,D]$};
\draw[dpgradweight] (dWK_mul.west) -- (dWK_out);

\draw[dpgradweight] (Sum_dBO) -- (dBO);
...
\draw[flow2] (T_AO_in) -- (dWO_calc.east)
  node[dimlabel, midway, above]{$[B,D,S]$};
\draw[dpgradweight] (dWO_calc) -- (dWO_GRAD);
...
% Legend and explanation
    \node[align=left, font=\scriptsize, text width=8cm] at (-5, -9.5) {
      \textcolor{red!70}{\textbf{Red gradients}} require \textbf{All-Reduce} before optimizer step\\
      (averaged across all Data Parallel nodes)\\[6pt]
      ...
      \quad • \textbf{Ring:} $2\frac{N_{DP}-1}{N_{DP}} \times (4D^2 + D)$ per node
    };
