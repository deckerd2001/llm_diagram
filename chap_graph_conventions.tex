...
$\mathbf{z} \in \mathbb{R}^{S}$ be the score vector over keys. The node
\textbf{S} computes
\[
...
\([B,N_H,S,S]\) are preserved. The forward pass typically caches
$\mathbf{p}$ (or $\mathbf{z}$).

\paragraph{Backward (dS)}

The backward node \textbf{dS} is the concrete instance of the abstract
operator $d f$ for $f = \mathrm{softmax}$. It implements
\[
  \mathrm{d}\mathbf{z}
  =
  dS(\mathbf{z}, \mathrm{d}\mathbf{p}),
\]
which, in terms of the Jacobian of softmax, is
\[
  dS(\mathbf{z}, \mathrm{d}\mathbf{p})
  =
  J_{\text{softmax}}(\mathbf{z})^{\mathsf T}\,\mathrm{d}\mathbf{p}.
\]
In practice this is computed using the standard softmax backward
formula; the diagrams treat dS as a single node whose input edges carry
$\mathbf{p}$ (or $\mathbf{z}$) and $\mathrm{d}\mathbf{p}$, and whose
output edge carries $\mathrm{d}\mathbf{z}$.
